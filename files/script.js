...
const squarePayBtn = document.querySelector(".place-order-btn");
...
function loadCheckoutPage() {
  ...
  if (savedCart.length === 0) {
    ...
    if (squarePayBtn) {
      squarePayBtn.textContent = "PAY NOW";
    }
  ...
  if (summaryItemsTotal) {
    summaryItemsTotal.textContent = `$${itemsTotal.toFixed(2)}`;
  }
  ...
  if (squarePayBtn) {
    squarePayBtn.textContent = `PAY $${grandTotal.toFixed(2)} NOW`;
  }
}
...
if (squarePayBtn) {
  squarePayBtn.addEventListener("click", async () => {
    ...
    squarePayBtn.textContent = "LOADING...";
